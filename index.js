...
fetch(url, options)
  ...
  .catch(err => console.error('error:' + err));
           ...
           case 'video':
            ...
            try {
                await fetchJson(`https://api.vhtear.com/youtube?query=${args.join(" ")}&apikey=ZEROKARA`)
                    .then(async(result) => {
                        await fetchJson(`https://api.vhtear.com/ytdl?link=https://youtu.be/${result.result[1].id}&apikey=ZEROKARA`)
                            .then(async(result) => {
                                caption = `\`❖ Title     :\` ${result.result.title}\n`
                                caption += `\`❖ Link      :\`${result.result.UrlVideo}\n`
                                caption += `\`❖ Size :\` ${result.result.size}\n`
                                await lol.replyWithPhoto({ url: result.result.imgUrl }, { caption: caption, parse_mode: "Markdown" })
                                await reply('wait ±2min')
                                soleh = `${result.result.title}kazuvid.mp4`
                                await lol.replyWithVideo({ url: result.result.UrlVideo, filename: soleh }, { thumb: result.result.imgUrl })
                            })
                    })
                } catch (e) {
                    console.log(e)
            ...
            default:
                if (!isGroup && !isCmd && !isMedia) {
                    await lol.replyWithChatAction("typing")
                    simi = await fetchJson(`http://simsumi.herokuapp.com/api?text=${encodeURI(body)}&lang=auto`)
