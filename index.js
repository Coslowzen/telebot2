...
const short = new shortio("coslow.tech", 247834, "G5fdSjGldgGj0xmP")
...
bot.on("message", async(lol) => {
    try {
        ...
        switch (command) {
            case 'help':
                 asw =  await fetchJson('https://api.waifu.pics/sfw/waifu')
                 text = `List Menu :
/trid (bahasa jepang)
/trjp (bahasa indo)
/play (judul lagu)
...
note = -command tanpa menggunakan tanda ( dan )
       -beberapa fitur sedang perbaikan
`
await lol.replyWithPhoto({ url: asw.url }, { caption: text })
                break
           case 'owner':
            ...
            case 'shid':
            query = args.join(" ")
            const url = `https://api.short.io/links/expand?domain=coslow.tech&path=${query}`
const options = {
  method: 'GET',
  headers: {Accept: 'application/json', Authorization: 'G5fdSjGldgGj0xmP'}
}

fetch(url, options)
  .then(res => res.json())
  .then(json => reply(json.idString))
  .catch(err => console.error('error:' + err));
           case 'sh':
           query = args.join(" ")
           short.createLink({ originalURL: query }).then(link => {
           reply(link.shortURL)
           })
           break
           case 'trid':
           if (args.length == 0) return reply(`Example: ${prefix + command} 今日`)
           query = args.join(" ")
           translate( query , {from: 'ja', to: 'id'}).then(res => {
           reply(res.text)
           })
           break
           case 'trjp':
           ...
           query = args.join(" ")
           translate( query , {from: 'id', to: 'ja'}).then(res => {
           reply(res.text)
           })
           break
           case 'cek':
           ...
           //translate//
            ...
            case 'wiki':
                ...
                try {
                  siang = args.join(" ")
                  sore = await fetchJson(`https://viko-api.herokuapp.com/api/info/wikipedia?search=${siang}&apikey=rxking`)
            ...
            //nulis
             case 'nulis':
                if (args.length == 0) return reply(`Example: ${prefix + command} halo semua`)
                     mi = args.join(" ")
                     hai = `https://api.vhtear.com/write?text=${mi}&apikey=ZEROKARA`
                     await reply('mohon ditunggu teks sedak di convert')
                     await lol.replyWithPhoto({ url: hai})
                     break
                // Downloader //
            case 'play':
                ...
                await fetchJson(`https://api-yogipw.herokuapp.com/api/yt/search?query=${args.join(" ")}`)
                    .then(async(result) => {
                        await fetchJson(`https://api-yogipw.herokuapp.com/api/download/ytmp3?url=${result.result[0].videoId}`)
                            .then(async(result) => {
                                caption = `\`❖ Title     :\` ${result.result.title}\n`
                                caption += `\`❖ Link      :\`${result.result.url}\n`
                                caption += `\`❖ Publish :\` ${result.result.published}\n`
                                ...
                                await lol.replyWithAudio({ url: result.result.url, filename: acmar }, { thumb: result.result.thumb })
                            })
                    })
               } catch (e) {
                    console.log(e)
           ...
           case 'video':
            ...
            try {
                await fetchJson(`https://api-yogipw.herokuapp.com/api/yt/search?query=${args.join(" ")}`)
                    .then(async(result) => {
                        await fetchJson(`https://api-yogipw.herokuapp.com/api/download/ytmp4?url=https://youtu.be/${result.result[0].videoId}`)
                            .then(async(result) => {
                                caption = `\`❖ Title     :\` ${result.result.title}\n`
                                caption += `\`❖ Link      :\`${result.result.url}\n`
                                caption += `\`❖ Publish :\` ${result.result.published}\n`
                                await lol.replyWithPhoto({ url: result.result.thumb }, { caption: caption, parse_mode: "Markdown" })
                                await reply('wait ±1min')
                                acmar = `${result.result.title}kazuvid.mp4`
                                await lol.replyWithVideo({ url: result.result.url, filename: acmar }, { thumb: result.result.thumb })
                            })
                    })
                } catch (e) {
                    console.log(e)
            ...
            default:
                if (!isGroup && !isCmd && !isMedia) {
                    await lol.replyWithChatAction("typing")
                    simi = await fetchJson(`https://api.simsimi.net/v2/?text=${encodeURI(body)}&lc=id`)
