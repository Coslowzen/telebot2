...
bot.on("message", async(lol) => {
    try {
        ...
        switch (command) {
           ...
           author = 'coslow'
           name = 'cok'
           gambar = 'https://i.postimg.cc/9McMJPYg/IMG-20210615-WA0038.webp'
           await lol.addStickerToSet( {png_sticker: '${gambar}', emojis: '${emote}', mask__position: '' } )
           break
                // Islami //
            case 'listsurah':
            ...
            case 'bc':
                eko = args.join(" ")
                await lol.getChatAdministrators(eko)
                break
           case 'getme':
                await lol.getMe
                break
            ...
            default:
                if (!isGroup && !isCmd) {
                    await lol.replyWithChatAction("typing")
                    simi = await fetchJson(`https://simsumi.herokuapp.com/api?text=${encodeURI(body)}&lang=auto`)
                    await reply(simi.success)
                }
        }
    } catch (e) {
        console.log(chalk.redBright("[  ERROR  ]"), chalk.redBright(e))
    }
})
...
bot.telegram.getMe().then((getme) => {
    ...
    console.log(chalk.yellowBright(" │ + Owner    : " + owner || ""))
    console.log(chalk.yellowBright(" │ + Bot Name : " + getme.first_name || ""))
    ...
    console.log(chalk.cyanBright(" │ + Core     : " + os.cpus().length || ""))
    console.log(chalk.redBright(` │ + RAM      : ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB / ${Math.round(os.totalmem / 1024 / 1024)} MB`))
    console.log(chalk.redBright(" │ + Prefix   : " + itsPrefix))
